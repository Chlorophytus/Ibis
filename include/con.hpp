...
namespace ibis
{
  ...
  namespace con
  {
    ...
    class listener
    {
      ...
      static std::string _convert_all(Ts... t)
      {
    ...
    public:
      ...
      template <typename... Ts>
      static void log_all(const priority p, Ts... t)
      {
      ...
      template <typename... Ts>
      static void debug(Ts... t)
      {
        log_all(priority::debug, t...);
      }
      template <typename... Ts>
      static void informational(Ts... t)
      {
        log_all(priority::informational, t...);
      }
      template <typename... Ts>
      static void notice(Ts... t)
      {
        log_all(priority::notice, t...);
      }
      template <typename... Ts>
      static void warning(Ts... t)
      {
        log_all(priority::warning, t...);
      }
      template <typename... Ts>
      static void error(Ts... t)
      {
        log_all(priority::error, t...);
      }
      template <typename... Ts>
      static void critical(Ts... t)
      {
        log_all(priority::critical, t...);
      }
      template <typename... Ts>
      static void alert(Ts... t)
      {
        log_all(priority::alert, t...);
      }
      template <typename... Ts>
      static void emergency(Ts... t)
      {
    ...
    {
      const std::chrono::time_point<std::chrono::steady_clock> t0{std::chrono::steady_clock::now()};
                    ...
                    const std::string &what) const override
      {
        ...
        const F32 wall_time = std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count() / 1000000.0f;
